...
public class Main extends JFrame implements ActionListener {
	...
	JMenuItem infQueryMenu=new JMenuItem("生鲜信息查询");
	JMenuItem shopQueryMenu=new JMenuItem("店铺信息查询");
	...
	Main(){
		...
		Menu0.add(menuExit);
		infQueryMenu.addActionListener(this);
		infQueryMenu.setEnabled(false);
		Menu2.add(infQueryMenu);
		shopQueryMenu.addActionListener(this);
		shopQueryMenu.setEnabled(false);
		Menu2.add(shopQueryMenu);
	...
	void showInfQueryPanel(){
		InfQueryPanel infQPanel=new InfQueryPanel();
		infQPanel.infList=this.infList;
		infQPanel.shopList=this.shopList;
		this.add(infQPanel, BorderLayout.CENTER);
		this.setVisible(true);
	}

	void showShopQueryPanel(){
		ShopQueryPanel shopQPanel=new ShopQueryPanel();
		shopQPanel.infList=this.infList;
		shopQPanel.shopList=this.shopList;
		this.add(shopQPanel, BorderLayout.CENTER);
		this.setVisible(true);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==infQueryMenu){
			this.getContentPane().removeAll();
			this.showInfQueryPanel();
		}
		if(e.getSource()==shopQueryMenu){
			this.getContentPane().removeAll();
			this.showShopQueryPanel();
		}
		...
		if(e.getSource()==menuOpen){
			...
			infQueryMenu.setEnabled(true);
			shopQueryMenu.setEnabled(true);
		...
		if(e.getSource()==menuChangePwd){
			...
			s=s.trim();
			if(s.length()==0){
				JOptionPane.showMessageDialog(null, "密码不能为空！", "栖霞区生鲜配送管理系统", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			for(int i=0;i<s.length();i++){
				...
				if(clearText.indexOf(""+c)==-1){
					JOptionPane.showMessageDialog(null, "密码中包含非法字符", "栖霞区生鲜配送管理系统", JOptionPane.ERROR_MESSAGE);
					return;
				}else{
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "密码修改成功！", "栖霞区生鲜配送管理系统", JOptionPane.INFORMATION_MESSAGE);
...
@SuppressWarnings("serial")
class InfQueryPanel extends JPanel implements ActionListener{
...
Vector<Information> infList=new Vector<Information>();
	...
	InfQueryPanel(){
		...
		searchBy.addItem("按生鲜名称查询");
		searchBy.addItem("按店铺名称查询");		
	...
	public void actionPerformed(ActionEvent arg0) {
		String str="种类名称\t店铺名称\t出厂日期\t生鲜单价\t月均销量\t顾客评分\n";
		for(int i=0;i<infList.size();i++){
			Information inf=(Information)this.infList.get(i);
			if(this.searchBy.getSelectedIndex()==0){
				if(inf.getFreshName().startsWith(this.keyword.getText().trim())||this.keyword.getText().trim().equals("")){
					str=str+inf.getFreshName()+"\t"+inf.getShopName()+"\t"+inf.getProducingTime()+"\t"+inf.getPrice()+"\t"+inf.getSales()+"\t"+inf.getGrade();
				}else
					continue;
			}if(this.searchBy.getSelectedIndex()==1){
				if(inf.getShopName().startsWith(this.keyword.getText().trim())||this.keyword.getText().trim().equals("")){
					str=str+inf.getFreshName()+"\t"+inf.getShopName()+"\t"+inf.getProducingTime()+"\t"+inf.getPrice()+"\t"+inf.getSales()+"\t"+inf.getGrade();
				}else
					continue;
			}
...
@SuppressWarnings("serial")
class ShopQueryPanel extends JPanel implements ActionListener{
	JComboBox searchBy=new JComboBox();
	JTextField keyword=new JTextField(10);
	JTextArea result=new JTextArea();
	Vector<Information> infList=new Vector<Information>();
